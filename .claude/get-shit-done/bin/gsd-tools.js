...
function roadmapPhaseInfoInternal(cwd, phaseNum) {
  ...
  try {
    ...
    const escapedPhase = normalized.replace(/\./g, '\\.');
    ...
    return {
      ...
      padded_phase: normalized.includes('.') ? normalized : normalized.padStart(2, '0'),
    };
  } catch {
    return null;
  }
...
function cmdInitPlanPhase(cwd, phase, includes, raw) {
  ...
  const config = loadConfig(cwd);
  const phaseInfo = findPhaseInternal(cwd, phase);
  const roadmapPhase = roadmapPhaseInfoInternal(cwd, phase);
  const resolvedPhaseNumber = phaseInfo?.phase_number || roadmapPhase?.phase_number || null;
  const resolvedPhaseName = phaseInfo?.phase_name || roadmapPhase?.phase_name || null;
  const resolvedPhaseSlug = phaseInfo?.phase_slug || roadmapPhase?.phase_slug || null;
  const resolvedPaddedPhase = phaseInfo?.phase_number?.padStart(2, '0') || roadmapPhase?.padded_phase || null;
  ...
  const result = {
    ...
    // Phase info
    phase_found: !!(phaseInfo || roadmapPhase),
    phase_dir: phaseInfo?.directory || null,
    phase_number: resolvedPhaseNumber,
    phase_name: resolvedPhaseName,
    phase_slug: resolvedPhaseSlug,
    padded_phase: resolvedPaddedPhase,
