...
find . -type f \( \
  -name "*.css" -o -name "*.scss" -o -name "globals.*" \
  -o -name "tailwind.config.*" -o -name "theme.*" \
  -o -name "tokens.*" -o -name "design-system.*" \
  -o -name "*.figma.json" \
\) 2>/dev/null | head -30

# Check for existing component directories
ls src/components/ 2>/dev/null || ls components/ 2>/dev/null || ls ui/ 2>/dev/null
```
...
```bash
cat package.json 2>/dev/null | grep -E '"react"|"next"|"vue"|"svelte"|"tailwind"|"vite"'
...
**Claude's role shifts when using 21st.dev:**
...
Remember: Claude is capable of extraordinary creative work. Don't hold back — show what can truly be created when thinking outside the box and committing fully to a distinctive vision. The goal is a UI that feels *deliberately designed*, not AI-generated.
