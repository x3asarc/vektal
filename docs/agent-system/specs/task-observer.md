...
4. **Detect oracle gap patterns: blocks in oracle_gaps field appearing 3+ times in 14 days → ImprovementProposal targeting graph sprint sync script** (v2 addition)
...
| **TO-DETECT-ORACLE-GAPS** | **Meta** | **Count oracle_gaps field occurrences per block across recent TaskExecutions. Threshold: 3+ in 14 days → ImprovementProposal.** | **—** |
