...
<task type="auto">
  <name>Task 3: Update pipeline to use vendor config</name>
  <files>src/core/enrichment/pipeline.py</files>
  <action>
Update EnrichmentPipeline to integrate vendor config:
...
Update run() method to call vendor rules after extraction:
```python
...
```
  ...
  <done>Pipeline applies vendor-specific rules from YAML automatically</done>
</task>
